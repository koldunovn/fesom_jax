#!/usr/bin/env python
"""CORE2 PROGNOSTIC-ICE gradient gate — Phase 6, Task 6.7 (GATE 6 AD re-check).

Re-runs the AD de-risking gate on the assembled CORE2 model WITH sea ice (EVP + FCT +
thermo). The per-kernel ice AD is already gated (the thermo Newton + freezing/albedo kinks
[6.2], the EVP Δ-singularity + the 120-subcycle scan [6.4], the FCT limiter [6.5]); this
confirms the SAME machinery survives in the **assembled** backward at CORE2 scale + measures
the memory (the EVP subcycle scan, nested in the time-loop scan, inflates it vs the Phase-5
37.8 GB).

Gates (smooth regimes only — the forced trajectory is non-smooth, Task 5.8):
* **[1] N=1 d(mean SST)/d(k_ver)** plateau — at step 1 ocean uv=0 ⇒ Kv=k_ver additively
  (the ocean vertical diffusion is smooth; the ice fluxes don't depend on k_ver).
* **[3] N d(mean SST)/d(T0)** finite EVERYWHERE incl. masked lanes — the strong masked-NaN
  probe on the ASSEMBLED ICE model (the backward flows through the thermo Newton, the EVP
  scan, the FCT limiter + every masked guard) + the checkpointed N-step **peak memory**.
* **[I] d(mean SST)/d(m_ice0)** finite + nonzero — the ice→ocean gradient path differentiates
  (ice volume → thermo flx → bc_T → SST).

Usage (GPU; scripts/archive/core2_ice_grad_gate.sbatch):  python scripts/archive/core2_ice_grad_gate.py --n 4
"""
...
def main():
    ...
    print(f"[setup] built in {time.time()-t0:.1f}s; N={args.n} ({args.n*DT/86400:.3f} days), ICE ON",
          flush=True)
    ...
    report_mem(f"T0-grad N={args.n} ICE")
    ...
    # [I] d(mean SST)/d(m_ice0) finite + nonzero — the ice→ocean gradient path.
    # ⚠️ Free [3]'s compiled executable first: 3 N-step backwards in one process OOM the
    # A100-40 (each compiles a fresh reverse graph). This is REDUNDANT with [3] (whose
    # backward already flows through the whole ice step — a/m_ice are in the scan carry); kept
    # as an explicit ice→ocean probe. On the 40 GB card run the probes as separate jobs.
    jax.clear_caches()
    ...
    print("ICE_GRAD_GATE_OK" if ok else "ICE_GRAD_GATE_FAIL", flush=True)

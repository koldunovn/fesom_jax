...
This reproduces it on CPU with 2 fake devices (seconds, no GPU) so the op can be bisected:
  ...
  python scripts/repro_sharded_grad_nan.py --nsteps 2 --config all3
...
Run:  JAX_PLATFORMS=cpu XLA_FLAGS=--xla_force_host_platform_device_count=2 python scripts/repro_sharded_grad_nan.py
"""

...
``scripts/paper/core2_paper_calib_gm_eki.py``); this script just stages the raw WOA18 **annual** T/S
...
Runs on the login node (CPU, just netCDF reads). Usage:  python scripts/tools/make_woa_ts_targets.py
"""
...
OUT = Path(__file__).resolve().parents[1] / "woa_ts_targets.npz"

...
#
# Args (optional):  $1 = years (default 2)   $2 = dt (default 1800)
# Usage:  sbatch scripts/core2_kpp_climate_gpu.sh        # 2 years, dt=1800, monthly means
...
YEARS=${1:-2}; DT=${2:-1800}
echo "=== $(hostname) $(date)  years=$YEARS dt=$DT (monthly means) ==="
nvidia-smi -L 2>/dev/null || echo "(no nvidia-smi)"
"$PY" scripts/core2_kpp_climate_run.py --years "$YEARS" --dt "$DT"
echo "exit: $?"

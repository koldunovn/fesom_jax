...
REHEARSE ON --sandbox FIRST. It is the same API against sandbox.zenodo.org, needs its own token
(sandbox.zenodo.org/account/settings/applications), and burns no real DOI. A 10.4 GB upload is not
something you want to discover a metadata mistake in.
...
def put_file(bucket_url: str, path: Path, token: str, retries: int = 3) -> dict:
    """Stream one file into the deposition bucket, with progress and retries.

    A plain requests.put() of a 10.4 GB file gives no progress and no recovery. We stream it with
    http.client and re-PUT on failure -- the bucket PUT is idempotent (it overwrites), so a retry
    is always safe.
    """
    parts = urllib.parse.urlparse(bucket_url)
    target = f"{parts.path}/{urllib.parse.quote(path.name)}?access_token={urllib.parse.quote(token)}"
        ...
        try:
            ...
            with open(path, "rb") as f:
                while chunk := f.read(4 << 20):
                    ...
                    sent += len(chunk)
                    now = time.time()
                    if now - last > 2 or sent == size:
                        last = now
                        rate = sent / max(now - t0, 1e-6) / 2**20
                        eta = (size - sent) / max(sent / max(now - t0, 1e-6), 1)
                        print(f"\r    {100 * sent / size:5.1f}%  "
                              f"{sent / 2**30:6.2f}/{size / 2**30:.2f} GB  "
                              f"{rate:6.1f} MB/s  ETA {eta / 60:5.1f} min", end="", flush=True)
            print()
            resp = conn.getresponse()
            body = resp.read()
            conn.close()
            if resp.status not in (200, 201):
                raise RuntimeError(f"HTTP {resp.status}: {body[:300].decode(errors='replace')}")
            return json.loads(body)
        except Exception as e:  # noqa: BLE001
            print(f"\n    attempt {attempt}/{retries} failed: {type(e).__name__}: {str(e)[:120]}")
            if attempt == retries:
                raise
...
def main() -> int:
    ...
    ap.add_argument("--deposition", type=int,
                    help="add to / update an EXISTING draft deposition instead of creating one "
                         "(use to resume after a failed upload)")
    args = ap.parse_args()
    ...
    print("files    :")
    for f in files:
        print(f"    {f.name:28s} {f.stat().st_size / 2**30:6.2f} GB")
    print(f"    {'TOTAL':28s} {total / 2**30:6.2f} GB")
    ...
    for f in files:
        if f.name in already:
            print(f"\n  {f.name}: already uploaded to this draft, skipping")
            continue
        print(f"\n  {f.name}  ({f.stat().st_size / 2**30:.2f} GB)")
        info = put_file(bucket, f, token)
        remote = (info.get("checksum") or "").replace("md5:", "")
        local = md5sum(f)
        if remote and remote != local:
            print(f"    !! checksum mismatch (zenodo {remote}, local {local})", file=sys.stderr)
            return 1
        print(f"    uploaded, md5 verified: {local}")

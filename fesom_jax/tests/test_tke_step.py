...
#
# ⚠️ XFAIL (a TRACKED gap, NOT a TKE-port bug): the JAX `build_core_forcing` step-1 wind
# stress at dt=1800 differs from the cdump's C-run by ~7e-4 (≈60% of the 1.2e-3 scale),
# IC-INDEPENDENT (identical to the digit under ic_core2 vs ic_core2_dist16) — a forcing
# step-1 date/time-convention mismatch, NOT FP and NOT the land-fill IC. (The KPP forcing
# gate matched <1e-12 at dt=500 with its own forcing-matched dump; the TKE cdump was
# generated by the C's TKE-vs-Fortran validation, whose dt=1800 step-1 forcing time the JAX
# convention doesn't reproduce.) The TKE PORT itself is bit-exact (JT.1 column core + JT.2
# driver wiring/geometry, replay-gated) — only this live forcing→assembly composition is
# blocked, pending the dt=1800 forcing-time alignment (a `core2_forcing` investigation, not
# TKE). Resolve by aligning the convention OR regenerating a forcing-matched TKE step dump.
# ---------------------------------------------------------------------------
_FORCING_GAP = pytest.mark.xfail(
    reason="dt=1800 step-1 forcing-time convention mismatch vs the cdump (forc_tke_surf "
           "7e-4, IC-independent); a core2_forcing alignment, not a TKE-port bug — the port "
           "is bit-exact via the JT.1/JT.2 replay gates", strict=False)


@_FORCING_GAP
def test_forc_tke_surf_matches_cdump(run1):
    """The assembled surface flux ``forc_tke_surf = |stress_node_surf|/ρ₀`` vs the cdump
    ``normstress`` (the forcing→driver-assembly seam). XFAIL: the dt=1800 forcing mismatch."""
    sfx = run1["sfx"]
...
def test_step1_tke_matches_cdump(run1):
    """End-of-step-1 ``state.tke`` vs the C cdump ``tke`` — the assembled live gate. XFAIL:
    downstream of the dt=1800 forcing mismatch above (tke is surface-flux-driven)."""

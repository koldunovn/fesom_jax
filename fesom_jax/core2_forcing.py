"""Assembled CORE2 surface forcing — Phase 5, Task 5.6.
...
* **Host SETUP** (non-differentiable): :func:`build_core_forcing` builds the readers +
  climatologies once; :meth:`CoreForcing.step_forcing` / :meth:`CoreForcing.stack` run
...
:func:`build_core_forcing` for the ``FESOM_CHL_SOURCE=None`` /
...
class CoreForcing:
    ...
    Build with :func:`build_core_forcing`; produce per-step forcing with
...
def build_core_forcing(mesh: Mesh, year: int, *, sst_ic=None,
    ...
    return CoreForcing(jra=jra, sss=sss, chl_clim=chl_clim,
